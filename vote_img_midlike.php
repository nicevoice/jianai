...
$imgPath = 'userRecord/'.$userID.'/'.$bagType.'/';
  ...
  // 检查是否重复选择
  $sql = "select userID, bagURL from user_midlikebag where userID=" .$userID." and bagURL='".$bagURL. "'"; 
  ...
  $path = $imgPath. 'midlike/';
  if(!is_dir($path))
     mkdir($path);
  
  $name = time().'.jpg';
  $path = $imgPath. 'midlike/'.$name; 
  copy($localPath, $path);
  
  
  // 把图片插入数据库  
   $sql = "insert into user_midlikebag(userID, bagType,bagName,mallID,bagPrice,localAddress,bagURL) values(".$userID. ",'".$bagType. "','".$bagName."',".$mall.",".$bagPrice.",'".$path."','".$bagURL."')"; 
