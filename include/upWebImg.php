...
class UPImages {
        ...
        function upLoad($inputName) 
		{
                ...
                $name = explode(".",$_POST[$inputName]);//将上传前的文件以"."分开取得文件类型
                ...
                $img_read_fd = get_img_Content($_POST[$inputName]); 
               
                if(file_put_contents($uploadFile, $img_read_fd)) {
                        $imgSize = getimagesize($uploadFile);
                        $width = $imgSize[0];
						$height = $imgSize[1];
					 if($width>$this->maxWidth || $height>$this->maxHeight) {
                                @unlink($uploadFile);
                                die("图片过大，请把您的图片缩小到500*600以内, <a href=\"../index.php\">返回</a>");
                        }  
               } else {
                    die("上传图片失败，请确认您输入了正确的图片网址 或上传时间超时,  <a href=\"../index.php\">返回</a>");
                      }
                return $photo;
        }
...
if(@$_GET["go"]) {
	  ...
	  /* ----------  上传文件  ---------------*/
	  $uploadFile = $img->upLoad("web_upImg");
	  $uploadPath = $root .  $uploadFile;
	  $imgInfo = getimagesize($uploadPath);
	  if(empty($imgInfo))
	       die("请确认您输入了正确的图片网址! <a href=\"../index.php\">返回</a>");
	  else
	   {				  
		 /* -------------------- 开始推荐 ----------------------------*/       
       	 $bagType = $_GET["type"];	
