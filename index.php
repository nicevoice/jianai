<?php
include ('header.php');
?>
<div class="well" align="center" style="background-color: white">
	<img height="240" src="images/icons/showcase.gif" />
</div>
<div class="row-fluid">
	<ul class="thumbnails">
		<li class="span3">
		    <a href="women.php">
    			<div class="thumbnail">
    				<img width="110" src="images/icons/women_bag.png" alt="">
    				<h4 align="center">女包</h4>
    			</div>
			</a>
		</li>
		<li class="span3">
		    <a href="men.php">
			<div class="thumbnail">
				<img width="110" src="images/icons/men_bag.png" alt="">
				<h4 align="center">男包</h4>
			</div>
			</a>
		</li>
		<li class="span3">
		    <a href="xiangbao.php">
			<div class="thumbnail">
				<img width="110" src="images/icons/luggage.jpeg" alt="">
				<h4 align="center">功能箱包</h4>
			</div>
			</a>
		</li>
		<li class="span3">
		    <a href="search.php">
			<div class="thumbnail">
				<img width="110" src="images/icons/search.png" alt="">
				<h4 align="center">图片搜索</h4>
			</div>
			</a>
		</li>
	</ul>
</div>
<?php
include ('footer.php');
?>

